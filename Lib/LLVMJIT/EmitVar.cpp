...
void EmitFunctionContext::local_get(GetOrSetVariableImm<false> imm)
{
	...
	push(irBuilder.CreateLoad(localPointers[imm.variableIndex]->getType()->getPointerElementType(), localPointers[imm.variableIndex]));
}
...
void EmitFunctionContext::global_get(GetOrSetVariableImm<true> imm)
{
	...
	if(globalType.isMutable)
	{
		...
		auto contextPointerValue = irBuilder.CreateLoad(contextPointerVariable->getType()->getPointerElementType(), contextPointerVariable);
		llvm::Value* globalPointer = irBuilder.CreateInBoundsGEP(contextPointerValue->getType()->getScalarType()->getPointerElementType(),
			contextPointerValue, {globalDataOffset});
