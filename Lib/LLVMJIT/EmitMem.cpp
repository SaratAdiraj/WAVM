...
static llvm::Value* getMemoryNumPages(EmitFunctionContext& functionContext, Uptr memoryIndex)
{
	...
	//auto *pointeeType = functionContext.getCompartmentAddress()->getType();
	...
	llvm::LoadInst* memoryNumPagesLoad = functionContext.loadFromUntypedPointer(
		functionContext.irBuilder.CreateInBoundsGEP(functionContext.getCompartmentAddress()->getType()->getScalarType()->getPointerElementType(),
											   ...
											   BoundsCheckOp boundsCheckOp)
{
	...
	else
	{
		...
		// the guard region.
		llvm::Type* endAddressType = functionContext.memoryInfos[memoryIndex].endAddressVariable->getType();
		llvm::Value* endAddress
			= irBuilder.CreateLoad(endAddressType->getPointerElementType(), functionContext.memoryInfos[memoryIndex].endAddressVariable);
														 ...
														 Uptr memoryIndex)
{
	llvm::Value* memoryBasePointer
		= irBuilder.CreateLoad(memoryInfos[memoryIndex].basePointerVariable->getType()->getPointerElementType(), memoryInfos[memoryIndex].basePointerVariable);
	llvm::Value* bytePointer = irBuilder.CreateInBoundsGEP(memoryBasePointer->getType()->getScalarType()->getPointerElementType(), memoryBasePointer, boundedAddress);
						 ...
						 Uptr numBytesLog2)
{
	...
	llvm::LoadInst* load = functionContext.irBuilder.CreateLoad(pointer->getType()->getPointerElementType(), pointer);
								...
								U64 numBytes)
{
	...
	else
	{
		...
		for(U32 vectorIndex = 0; vectorIndex < numVectors; ++vectorIndex)
		{
			auto functionVal = functionContext.irBuilder.CreateInBoundsGEP(pointer->getType()->getScalarType()->getPointerElementType(),
					pointer, {emitLiteral(functionContext.llvmContext, U32(vectorIndex))});
			auto load
				= functionContext.irBuilder.CreateLoad(functionVal->getType()->getPointerElementType(), functionVal);
	...
	else
	{
		for(U32 vectorIndex = 0; vectorIndex < numVectors; ++vectorIndex)
		{
			...
			auto store = functionContext.irBuilder.CreateStore(
				interleavedVector,
				functionContext.irBuilder.CreateInBoundsGEP(pointer->getType()->getScalarType()->getPointerElementType(),
					pointer, {emitLiteral(functionContext.llvmContext, U32(vectorIndex))}));
			store->setVolatile(true);
			store->setAlignment(LLVM_ALIGNMENT(1));
		}
